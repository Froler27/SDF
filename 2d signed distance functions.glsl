...
{
    offset.x = float(int(iResolution.x/6.));
    offset.y = float(int(iResolution.y/3.));
	vec2 p = fragCoord.xy + vec2(.5) - offset;
	...
	// gradient
	//vec4 col = vec4(0.5, 0.5, 0.5, 1.0) * (1.0 - length(c - p -offset)/iResolution.x);
    vec4 col = vec4(0.5, 0.5, 0.5, 1.0);
	// grid
	col *= clamp(min(mod(p.y, 10.0), mod(p.x, 10.0)), 0.9, 1.);
	// ambient occlusion
	...
	// shape outline
	//col = mix(col, vec4(0.1, 0.1, 0.1, 1.0), innerBorderMask(dist, 3.5));
